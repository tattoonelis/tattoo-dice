...
async function loadAvailableThemes(){
  const fallback=[...themeSelect.options].map(option=>({id:option.value,name:option.textContent}));
  let themes=fallback;
  try{themes=await fetchCanonThemes({supabaseUrl:SUPABASE_URL,supabaseKey:SUPABASE_KEY,fallback});}catch{}
  const preferred=themes.some(theme=>theme.id===themeSelect.value)?themeSelect.value:themes[0]?.id||"classic";
  themeSelect.innerHTML="";
  themes.forEach(theme=>{const option=document.createElement("option");option.value=theme.id;option.textContent=theme.name;themeSelect.append(option);});
